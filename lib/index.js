...
module.exports = class {
    ...
    async get(key) {
        ...
        const compressedResult = await this.client.getBuffer(this.generateKey(key));
        if (!compressedResult) {
            return null;
        }

        try {
            var result = LZ4.decode(compressedResult);
            var envelope = JSON.parse(result.toString());
        }
    ...
    set(key, value, ttl) {
        ...
        const compressedEnvelope = LZ4.encode(Buffer.from(stringifiedEnvelope));
        return this.client.psetex(cacheKey, ttl, compressedEnvelope);
    }
